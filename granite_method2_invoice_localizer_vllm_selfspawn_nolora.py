...
------------------------------------------
The model is served as a native multimodal LoRA in vLLM.  Therefore the
self-managed vLLM backend automatically adds:
  --enable-lora
  --max-lora-rank 256
  --default-mm-loras '{"image": "ibm-granite/granite-vision-4.1-4b"}'
You normally do not need to pass these manually.
"""
...
class ManagedVLLMGraniteBackend(OpenAICompatibleGraniteBackend):
    ...
    def _start_vllm_process(
        ...
        # Granite Vision 4.1 4B is natively supported by recent vLLM and the
        # official HF model-card command is simply:
        #     vllm serve ibm-granite/granite-vision-4.1-4b
        # Do NOT add --default-mm-loras for this model.  That makes vLLM treat the
        # full Granite Vision checkpoint as a LoRA adapter; then adapter weights
        # such as image_newline are matched against the wrong base wrapper and vLLM
        # raises: "There is no module or parameter named 'image_newline'...".
        is_granite_vision_41 = "granite-vision-4.1-4b" in model_id.lower()
        if granite_vision_lora and is_granite_vision_41:
            warnings.warn(
                "Ignoring Granite Vision LoRA flags for granite-vision-4.1-4b. "
                "This model should be served directly, not via --default-mm-loras."
            )

        # Conservative startup defaults for this VLM.  These do not change model
        # semantics; they just reduce KV-cache reservation and batching pressure.
        if is_granite_vision_41:
            if max_model_len is None and "--max-model-len" not in cmd:
                cmd += ["--max-model-len", "4096"]
            if gpu_memory_utilization is None and "--gpu-memory-utilization" not in cmd:
                cmd += ["--gpu-memory-utilization", "0.70"]
            if "--max-num-seqs" not in cmd:
                cmd += ["--max-num-seqs", "1"]
        ...
        env = os.environ.copy()
        # Make vLLM startup failures visible immediately in vllm_server.log.
        env.setdefault("PYTHONUNBUFFERED", "1")

        # Granite Vision 4.1 4B currently tends to be more reliable through vLLM's
        # V0 engine path in mixed multimodal-LoRA setups.  Recent vLLM releases enable
        # the V1 engine by default, and the V1 multiprocess startup error often ends
        # with the unhelpful message: "Engine core initialization failed ...".
        # Setting VLLM_USE_V1=0 is the official way to force the V0 path.
        if use_v1 is None:
            if granite_vision_lora and "granite-vision-4.1-4b" in model_id.lower():
                env["VLLM_USE_V1"] = "0"
        else:
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--dtype", type=str, default="auto", choices=["auto", "bfloat16", "bf16", "float16", "fp16", "float32", "fp32"], help="vLLM/Transformers dtype. Default auto is safer across GPU types; use float16 on V100/T4 if BF16 is unsupported.")
    ...
    ap.add_argument(
        "--no-vllm-granite-vision-lora",
        dest="vllm_granite_vision_lora",
        action="store_false",
        default=True,
        help=(
            "Disable Granite Vision 4.1 native LoRA serving flags. Keep this enabled for "
            "ibm-granite/granite-vision-4.1-4b, because the vision model is served in vLLM "
            "as a multimodal LoRA adapter."
        ),
    )
    ...
    ap.add_argument(
        ...
        default="auto",
        help=(
            "Control vLLM engine generation for the spawned server. Default 'auto' forces "
            "VLLM_USE_V1=0 for Granite Vision 4.1 4B, because V1 can fail during "
            "multimodal-LoRA startup on some vLLM/GPU environments. Use '1' to force V1."
        ),
    )
    ap.add_argument(
    ...
    ap.add_argument(
        ...
        default=None,
        help=(
            "JSON string for vLLM --default-mm-loras. Default for Granite Vision is "
            "{\"image\": <model-id>}."
        ),
    )
