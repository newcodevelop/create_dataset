#!/usr/bin/env python3
"""
vLLM SROIE benchmark inference with OCR-nudged semantic grounding.

What this script adds over a plain VLM inference script:

1) If the VLM predicts a field value:
   - Treat the VLM value as the semantic target.
   - Check whether OCR text inside the VLM bbox supports that value.
   - If the bbox is bad or absent, retrieve the best OCR line span globally.
   - Return the recovered/refined evidence bbox.

2) If the VLM predicts an empty value:
   - Use primitive field-conditioned OCR-span proposal.
   - Generate contiguous OCR windows, score them for the requested field,
     prune boundary noise, and use the resulting OCR span as the evidence box.
   - Optionally ask the VLM on the crop to verify/extract the field.

Core principle:
    VLM predicts WHAT when it can; OCR evidence retrieval finds WHERE.
    When VLM predicts nothing, OCR-span retrieval proposes a primitive WHERE.

Expected SROIE layout:
    dataset_root/
      img/img/*.jpg
      box/*.txt                # OCR line boxes: x1,y1,x2,y2,x3,y3,x4,y4,text
      entities/*.txt           # GT JSON with company/address/date/total
"""
...
DEFAULT_FIELDS = "company,address,date,total"
...
def apply_model_preset(args: argparse.Namespace) -> None:
    ...
    if args.output_json is None:
        args.output_json = f"SROIE_{slug}_grounded_results.json"
    if args.output_csv is None:
        args.output_csv = f"SROIE_{slug}_grounded_results.csv"
    if args.grounding_csv is None:
        args.grounding_csv = f"SROIE_{slug}_grounding_details.csv"
    if args.vllm_log is None:
        args.vllm_log = f"vllm_{slug}_sroie_grounded_server.log"

# This prompt asks for both values and rough bboxes. The grounding code below
# remains robust when the model returns the old flat schema without bboxes.
GROUNDED_SROIE_PROMPT = """
You are an OCR extraction and grounding system.

Extract the four SROIE receipt fields from this image.
Return ONLY valid JSON in this exact schema:

{
  "company": {"value": "", "bbox": [0, 0, 0, 0]},
  "address": {"value": "", "bbox": [0, 0, 0, 0]},
  "date": {"value": "", "bbox": [0, 0, 0, 0]},
  "total": {"value": "", "bbox": [0, 0, 0, 0]}
}
...
- Use exact OCR text when visible.
- If a field is missing, set value to "" and bbox to [0, 0, 0, 0].
- Bbox format is [x1, y1, x2, y2] around the smallest visible evidence region.
- Coordinates must be pixel coordinates in the provided image.
- total is the final payable amount, not subtotal, tax, cash, or change.
""".strip()
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Benchmark vLLM-supported VLM/OCR models on SROIE with OCR-nudged semantic grounding."
    )
    parser.add_argument(
        "--model_preset",
    ...
    parser.add_argument(
        "--dataset_root",
        default=os.environ.get("SROIE_ROOT", os.path.join(OCR_BASELINES_DIR, "SROIE dataset")),
    )
    parser.add_argument("--img_dir", default=os.environ.get("IMG_DIR"))
    parser.add_argument("--gt_dir", default=os.environ.get("GT_DIR"))
    parser.add_argument("--ocr_dir", default=os.environ.get("OCR_DIR"))
    parser.add_argument(
        "--output_json",
    ...
    parser.add_argument(
        ...
        help=(
            "grounded asks for value+bbox; flat asks for four SROIE values; "
            "vllm/vllm_raw preserve older prompt styles."
        ),
    )

    parser.add_argument("--max_tokens", type=int, default=int(os.environ.get("MAX_TOKENS", "512")))
    ...
    parser.add_argument(
        ...
        help="Cap total crop-level VLM calls across the run.",
    )
    parser.add_argument(
        "--crop_padding",
        type=int,
    ...
    if args.ocr_dir is None:
        # args.ocr_dir = os.path.join(args.dataset_root, "box")
        args.ocr_dir = os.path.join(args.dataset_root, "box_pred")
...
def normalize_field(field: str, value: Any) -> str:
    ...
    if field == "total":
        text = text.replace(",", "")
        text = re.sub(r"\b(rm|rp|idr|usd|rs|myr|inr)\b", "", text)
...
def prompt_text() -> str:
    if ARGS.prompt_style == "grounded":
        return GROUNDED_SROIE_PROMPT
    if ARGS.prompt_style == "flat":
        return FLAT_SROIE_PROMPT
...
def crop_verification_prompt(field: str) -> str:
    readable = {
        ...
        "total": "final payable total amount",
    }.get(field, field)
    field_rules = {
        "company": """
- Return only the merchant/supplier/company name.
- Exclude customer names, cardholder names, owner labels, registration numbers, addresses, tax IDs, phone/email, and receipt titles.
- If the crop contains only a customer/person name and not the merchant, set supports_field to false.
""".strip(),
        "address": """
- Return only the merchant/supplier address lines.
- Exclude the company name, registration/company numbers, GST/SST/tax IDs, TEL/FAX/phone, email, website, receipt number, invoice labels, date/time, and item lines.
- Keep branch/location text only when it is visibly part of the address block.
""".strip(),
        "date": """
- Return only the receipt/invoice date.
- Exclude time, cashier, receipt number, invoice number, and labels unless they are inseparable from the date text.
""".strip(),
        "total": """
- Return only the final payable total amount.
- Exclude subtotal, tax, rounding, cash paid, change, item prices, and labels.
""".strip(),
    }.get(field, "- Return only the requested target field value.")
    return f"""
...
def sroie_guided_json_schema() -> Dict[str, Any]:
    if ARGS.prompt_style == "grounded":
        field_schema = {
            "type": "object",
            "properties": {
        ...
        return {
            "type": "object",
            "properties": {field: field_schema for field in ["company", "address", "date", "total"]},
...
def optional_generation_params() -> Dict[str, Any]:
    ...
    if ARGS.guided_json:
        params["guided_json"] = sroie_guided_json_schema()
    ...
    if content.startswith("{") or content.startswith("["):
        try:
            ...
            for idx, item in enumerate(items):
                ...
                bbox = sanitize_bbox(item.get("bbox") or item.get("box"), None)
                if text and bbox:
...
def money_amounts(text: str) -> List[str]:
    ...
    patterns = [
        r"\b(?:rm|myr|usd|rs|inr|rp)?\s*\d{1,6}(?:[,.]\d{2})\b",
        r"\b\d{1,6}(?:[,.]\d{2})\s*(?:rm|myr|usd|rs|inr|rp)?\b",
    ]
    found: List[str] = []
    for p in patterns:
...
def contamination_penalty(text: str, field: str) -> float:
    ...
    if field in ("company", "address"):
        bad_words = ["total", "subtotal", "tax", "gst", "invoice", "cash", "change", "qty", "quantity", "price", "amount"]
        penalty += 0.10 * sum(1 for w in bad_words if re.search(rf"\b{re.escape(w)}\b", t))
        if "tel" in t or "fax" in t or "phone" in t:
            penalty += 0.12
    if field == "address":
        if "gst" in t or "tax" in t:
            penalty += 0.20
        if len(money_amounts(t)) >= 2:
            penalty += 0.20
    if field == "company":
        if has_date_pattern(t):
            penalty += 0.20
        if len(money_amounts(t)) > 0:
            penalty += 0.20
    if field == "date":
        if "total" in t or len(money_amounts(t)) > 0:
            penalty += 0.18
    if field == "total":
        if "change" in t or "cash" in t:
            penalty += 0.25
...
def field_semantic_score(text: str, field: str, start: int = 0, num_lines: int = 1, total_lines: int = 1) -> float:
    ...
    if field == "address":
        ...
        marker_hits = sum(1 for m in address_markers if re.search(rf"\b{re.escape(m)}\b", t))
        score += min(0.40, 0.10 * marker_hits)
        ...
        marker_hits = sum(1 for m in company_markers if re.search(rf"\b{re.escape(m)}\b", t))
        score += min(0.45, 0.15 * marker_hits)
        ...
        if has_date_pattern(t):
            score += 0.75
        if any(m in t for m in ["date", "dated", "time"]):
            score += 0.12
        if len(tokens) <= 8:
            score += 0.08

    elif field == "total":
        if re.search(r"\b(total|grand total|amount due|balance due|net total|payable)\b", t):
            score += 0.45
        amounts = money_amounts(t)
        if amounts:
            score += 0.30
        if re.search(r"\b(rm|myr|usd|rs|inr|rp)\b", t):
            score += 0.10
        if total_lines > 0 and start / max(1, total_lines) > 0.45:
            score += 0.05
        # Prefer compact total region instead of huge tax summaries.
        if len(amounts) > 3:
            score -= 0.10

    else:
        # Generic field: prefer compact text spans.
        if len(tokens) >= 1:
            score += 0.20
...
def score_span_for_value(span: Dict[str, Any], field: str, value: str) -> float:
    return fuzzy_similarity(span.get("text", ""), normalize_field(field, value) if field == "total" else value)
...
def ground_field(
    ...
    if value:
        ...
        inside_score = fuzzy_similarity(inside_text, normalize_field(field, value) if field == "total" else value)
...
def serialize_fields(values: Dict[str, Any], fields: List[str]) -> str:
    rows = []
    for field in fields:
        rows.append(f"{field}: {normalize_field(field, values.get(field, ''))}")
    return "\n".join(rows)
...
def main() -> None:
    image_paths = sorted(
        ...
        + glob(os.path.join(ARGS.img_dir, "*.jpeg"))
    )
    ...
    for idx, image_path in enumerate(image_paths):
        ...
        gt_path = os.path.join(ARGS.gt_dir, file_id + ".txt")
        ocr_path = os.path.join(ARGS.ocr_dir, file_id + ".txt")
        ...
        try:
            ...
            print("GROUND TRUTH LOAD FAILED")
            print(exc)
            continue

        ocr_lines = load_ocr_lines(ocr_path)
        print(f"OCR lines loaded: {len(ocr_lines)} from {ocr_path}")
        ...
        vlm_field_values: Dict[str, str] = {}
        vlm_field_bboxes: Dict[str, Optional[List[float]]] = {}
        grounded_fields: Dict[str, Dict[str, Any]] = {}
        final_values: Dict[str, str] = {}

        for field in EVAL_FIELDS:
            value, bbox = extract_field_prediction(pred_json, field, sent_size, original_size)
            vlm_field_values[field] = value
            vlm_field_bboxes[field] = bbox
            grounded = ground_field(image_path, field, value, bbox, ocr_lines)
            grounded_fields[field] = grounded
            final_values[field] = str(grounded.get("final_value") or "").strip()

            print(
                f"GROUNDING {field}: status={grounded.get('status')} "
                f"score={grounded.get('evidence_score', 0.0):.3f} "
                f"value={final_values[field]!r}"
            )
            if grounded.get("final_bbox") is not None:
                print(f"  bbox={grounded.get('final_bbox')} evidence={grounded.get('evidence_text')!r}")
        ...
        for field in EVAL_FIELDS:
            gt_value = normalize_field(field, gt.get(field, ""))
            pred_value = normalize_field(field, final_values.get(field, ""))
            tp, fp, fn = token_multiset_counts(gt_value, pred_value)
            scores = prf(tp, fp, fn)
            exact = gt_value == pred_value
            if exact:
                exact_fields += 1
            doc_tp += tp
            doc_fp += fp
            ...
            field_scores[field] = {
                ...
                "pre_crop_pred": grounded_fields[field].get("pre_crop_final_value"),
                "pred": final_values.get(field, ""),
                "normalized_gt": gt_value,
                "normalized_pred": pred_value,
                "exact_match": exact,
                "token_counts": {"tp": tp, "fp": fp, "fn": fn},
                "token_metrics": scores,
                "grounding_status": grounded_fields[field].get("status"),
                "grounding_bbox": grounded_fields[field].get("final_bbox"),
                "evidence_text": grounded_fields[field].get("evidence_text"),
                "evidence_score": grounded_fields[field].get("evidence_score"),
                "crop_verification": grounded_fields[field].get("crop_verification"),
            }
...
def write_csv_outputs(all_results: List[Dict[str, Any]]) -> None:
    ...
    with open(ARGS.grounding_csv, "w", newline="", encoding="utf-8") as csvfile:
        ...
        for result in all_results:
            for field, vals in result.get("grounded_fields", {}).items():
...
def print_base_summary(all_results: List[Dict[str, Any]]) -> None:
    ...
    field_totals: Dict[str, Dict[str, int]] = {
        field: {"exact": 0, "total": 0, "tp": 0, "fp": 0, "fn": 0} for field in EVAL_FIELDS
    }
    ...
    for result in all_results:
        ...
        for field in EVAL_FIELDS:
            gt_value = normalize_field(field, gt.get(field, ""))
            pred_value = normalize_field(field, base_values.get(field, ""))
            tp, fp, fn = token_multiset_counts(gt_value, pred_value)
            exact = gt_value == pred_value
    ...
    print("\n====================================================")
    print(f"FINAL {ARGS.model_label} SROIE BASE SUMMARY")
    print("====================================================\n")
...
def print_summary(all_results: List[Dict[str, Any]]) -> None:
    ...
    for result in all_results:
        for field, vals in result["field_scores"].items():
            ...
            status_counts[str(vals.get("grounding_status"))] += 1

    print("\n====================================================")
    print(f"FINAL {ARGS.model_label} SROIE GROUNDED SUMMARY")
    print("====================================================\n")
    print(f"Documents processed : {docs_processed}")
    print(f"JSON parse OK       : {parse_ok}/{docs_processed}")
    print("\nMacro token metrics over documents:")
