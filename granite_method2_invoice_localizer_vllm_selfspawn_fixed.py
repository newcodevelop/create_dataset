...
class ManagedVLLMGraniteBackend(OpenAICompatibleGraniteBackend):
    ...
    def _start_vllm_process(
        ...
        # Granite Vision 4.1 4B is served by vLLM as a native multimodal LoRA.
        # Without these three flags, vLLM can exit during startup before the
        # OpenAI endpoint appears, which surfaces as ConnectionRefusedError here.
        # IBM's serving recipe uses exactly this pattern:
        #   --enable-lora --max-lora-rank 256
        #   --default-mm-loras '{"image": "ibm-granite/granite-vision-4.1-4b"}'
        if granite_vision_lora and "granite-vision-4.1-4b" in model_id.lower():
            mm_loras = default_mm_loras or json.dumps({"image": model_id})
            if "--enable-lora" not in cmd:
                cmd += ["--enable-lora"]
            if "--max-lora-rank" not in cmd:
                cmd += ["--max-lora-rank", str(max_lora_rank)]
            if "--default-mm-loras" not in cmd:
                cmd += ["--default-mm-loras", mm_loras]
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--vllm-max-lora-rank", type=int, default=256, help="Max LoRA rank for Granite Vision native LoRA serving.")
    ap.add_argument(
