...
Note for Granite Vision 4.1 4B with vLLM
------------------------------------------
The self-managed vLLM path now follows the working launch style used in
benchmark_sroie_grounded.py:
  vllm serve ibm-granite/granite-vision-4.1-4b \
    --served-model-name granite-vision-41-4b \
    --trust-remote-code \
    --limit-mm-per-prompt '{"image": 1}'
No --default-mm-loras flags are added.
"""
...
class ManagedVLLMGraniteBackend(OpenAICompatibleGraniteBackend):
    ...
    def __init__(
        ...
        trust_remote_code: bool = True,
        granite_vision_lora: bool = True,
        max_lora_rank: int = 256,
        default_mm_loras: Optional[str] = None,
        use_v1: Optional[bool] = None,
        disable_v1_multiprocessing: bool = False,
        extra_args: str = "",
        ...
        self.runtime_cache_dir = runtime_cache_dir
        ...
        client_model_name = served_model_name or ("granite-vision-41-4b" if "granite-vision-4.1-4b" in model_id.lower() else model_id)
        ...
        if endpoint_already_ready:
            ...
            self._start_vllm_process(
                ...
                granite_vision_lora=granite_vision_lora,
                max_lora_rank=max_lora_rank,
                default_mm_loras=default_mm_loras,
                use_v1=use_v1,
                disable_v1_multiprocessing=disable_v1_multiprocessing,
    ...
    def _start_vllm_process(
        ...
        granite_vision_lora: bool,
        max_lora_rank: int,
        default_mm_loras: Optional[str],
        use_v1: Optional[bool],
        disable_v1_multiprocessing: bool,
        extra_args: str,
        log_file: Optional[Path],
    ) -> None:
        """Spawn vLLM using the same pattern as the working benchmark script.

        The important change compared with the earlier versions is that Granite
        Vision 4.1 4B is served as the checkpoint itself, not as
        --default-mm-loras.  The benchmark code that works for Granite uses:
        ...
        This method mirrors that behavior while keeping the rest of the Method-2
        extraction/translation/rendering logic untouched.
        """
        is_granite_vision_41 = "granite-vision-4.1-4b" in model_id.lower()

        # `serve_command` may be either "vllm serve" or an absolute command such
        # as "/path/to/vllm serve".  shlex.split keeps quoted arguments intact.
        cmd = shlex.split(serve_command) + [model_id]

        # Host/port and core resource arguments are the same structure used by
        # the working benchmark_sroie_grounded.py launcher.
        ...
        # Use the user-provided alias, or the known working Granite alias.  This
        # alias is also used by the OpenAI client request payload.
        if served_model_name:
            cmd += ["--served-model-name", served_model_name]
        elif is_granite_vision_41:
            cmd += ["--served-model-name", "granite-vision-41-4b"]

        # dtype/max-model-len are optional exactly like the benchmark.  Passing
        # them only when the caller asks avoids tripping vLLM-version-specific
        # behavior.  The script default dtype is now None for vLLM.
        if dtype and dtype not in {"default", "none", "auto"}:
            dtype_for_vllm = {"bf16": "bfloat16", "fp16": "float16", "fp32": "float32"}.get(dtype, dtype)
            cmd += ["--dtype", dtype_for_vllm]
        if max_model_len is not None:
            cmd += ["--max-model-len", str(max_model_len)]

        # Granite needs trust-remote-code in the working benchmark preset.
        if trust_remote_code:
            cmd += ["--trust-remote-code"]

        # The working Granite preset includes this multimodal request limit.
        # Add it unless the user already supplied a custom value in extra_args.
        if is_granite_vision_41 and "--limit-mm-per-prompt" not in extra_args:
            cmd += ["--limit-mm-per-prompt", '{"image": 1}']

        # IMPORTANT: do not append --enable-lora / --default-mm-loras here.
        # Your observed `image_newline` error comes from treating the full
        # Granite Vision checkpoint as a LoRA adapter.
        if granite_vision_lora or default_mm_loras:
            warnings.warn(
                "Ignoring vLLM LoRA/default-mm-loras options. The working Granite "
                "launcher serves ibm-granite/granite-vision-4.1-4b directly."
            )
        ...
        # Do not force VLLM_USE_V1 by default.  The benchmark launcher leaves vLLM
        # to choose its own engine.  Still allow explicit override for debugging.
        if use_v1 is not None:
            env["VLLM_USE_V1"] = "1" if use_v1 else "0"
        if disable_v1_multiprocessing:
            env["VLLM_ENABLE_V1_MULTIPROCESSING"] = "0"

        # If --device cuda:N is passed, bind the spawned server to that GPU.
        m = re.fullmatch(r"cuda:(\d+)", device.strip())
        if m:
            env["CUDA_VISIBLE_DEVICES"] = m.group(1)

        # Match the benchmark's cache handling.  This avoids permission issues in
        # container/pod environments and keeps HF/vLLM/Triton caches under a
        # writable directory when the user supplies --hf-cache-dir.
        if getattr(self, "hf_cache_dir", None):
        ...
        if getattr(self, "runtime_cache_dir", None):
            runtime_cache_dir = str(self.runtime_cache_dir)
            ...
            for key in [
                ...
                "CUDA_CACHE_PATH", "VLLM_CACHE_ROOT", "TMPDIR",
            ]:
                os.makedirs(env[key], exist_ok=True)

        if log_file is not None:
            log_file.parent.mkdir(parents=True, exist_ok=True)
            # Open with "w" like the benchmark so each run has a clean log.
            self._log_fh = log_file.open("w", encoding="utf-8")
            stdout = self._log_fh
    ...
    def _endpoint_ready_quietly(self) -> bool:
        # The working benchmark checks /v1/models.  Some vLLM builds do not expose
        # /health consistently, so prefer /v1/models here too.
        try:
    ...
    def _wait_until_ready(self, startup_timeout: float) -> None:
        print(f"[vLLM] Waiting for endpoint at {self.models_url} ...")
        start = time.time()
        last_err = ""
        ...
        tail = self._tail_log()
        raise TimeoutError(
            f"Timed out after {startup_timeout:.0f}s waiting for vLLM at {self.models_url}.\n"
            f"Last error: {last_err}\n"
            f"Log tail:\n{tail}"
        )
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--dtype", type=str, default=None, choices=[None, "auto", "bfloat16", "bf16", "float16", "fp16", "float32", "fp32"], help="Optional vLLM/Transformers dtype. For vLLM, omit by default to match the working Granite benchmark launcher; pass float16 if your setup needs it.")
    ...
    ap.add_argument("--hf-cache-dir", type=str, default=os.environ.get("HF_CACHE_DIR"), help="Optional Hugging Face cache directory for the spawned vLLM server.")
    ap.add_argument("--runtime-cache-dir", type=str, default=os.environ.get("RUNTIME_CACHE_DIR"), help="Optional runtime cache directory for Torch/Triton/vLLM temporary files.")
    ...
    ap.add_argument("--vllm-served-model-name", type=str, default="granite-vision-41-4b", help="Alias exposed by vLLM and used by the OpenAI client. Default matches the working Granite benchmark.")
    ...
    ap.add_argument(
        "--vllm-granite-vision-lora",
        dest="vllm_granite_vision_lora",
        action="store_true",
        default=False,
        help=(
            "Deprecated compatibility flag. It is ignored by the current launcher because "
            "the working Granite benchmark serves the checkpoint directly, without --default-mm-loras."
        ),
    )
    ap.add_argument("--vllm-max-lora-rank", type=int, default=256, help="Legacy LoRA option; normally unused for granite-vision-4.1-4b.")
    ap.add_argument(
        "--vllm-use-v1",
        choices=["auto", "0", "1"],
        default="auto",
        help=(
            "Control VLLM_USE_V1 for the spawned server. Default 'auto' does not set it, "
            "matching the working benchmark launcher. Use 0 or 1 only for debugging."
        ),
    )
    ap.add_argument(
        "--vllm-disable-v1-multiprocessing",
        action="store_true",
        help="Set VLLM_ENABLE_V1_MULTIPROCESSING=0. Mostly useful only when forcing --vllm-use-v1 1 for debugging.",
    )
    ap.add_argument(
        "--vllm-default-mm-loras",
        type=str,
        default=None,
        help=(
            "Deprecated compatibility option. Ignored by default because the working Granite "
            "launcher does not use --default-mm-loras."
        ),
    )
...
def main() -> None:
    ...
    if args.backend == "transformers":
        ...
        vllm_use_v1 = None if args.vllm_use_v1 == "auto" else (args.vllm_use_v1 == "1")
        backend = ManagedVLLMGraniteBackend(
            ...
            granite_vision_lora=args.vllm_granite_vision_lora,
            max_lora_rank=args.vllm_max_lora_rank,
            default_mm_loras=args.vllm_default_mm_loras,
            use_v1=vllm_use_v1,
            disable_v1_multiprocessing=args.vllm_disable_v1_multiprocessing,
            ...
            runtime_cache_dir=Path(args.runtime_cache_dir) if args.runtime_cache_dir else None,
        )
